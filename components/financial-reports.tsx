...
function StatementSection({title,rows,totalLabel=title}:{title:string;rows:FinancialTransaction[];totalLabel?:string}){
  ...
  return <section className="overflow-hidden rounded-xl border border-[#203047] bg-[#0b1726] shadow-sm">
    ...
    {groups.map(group=>{const items=rows.filter(r=>clean(r.report_category||r.description)===group);return <div key={group}><div className="flex items-center justify-between border-y border-[#203047] bg-[#0d1928] px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400"><span>{group}</span><span className="tabular-nums text-slate-300">{money(total(items))}</span></div><AccountRows rows={items}/></div>})}
...
export function NeracaReport(){
  ...
  if(!rows.length)return <Empty report="Neraca"/>;
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-xl border border-[#203047] bg-[#0a1522] p-4 md:p-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <StatementSection title="Aset" rows={assets} totalLabel="TOTAL ASET"/>
        <div className="space-y-4"><StatementSection title="Liabilitas" rows={liabilities} totalLabel="TOTAL LIABILITAS"/><StatementSection title="Ekuitas" rows={equity} totalLabel="TOTAL EKUITAS"/><div className="flex items-center justify-between rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-4 text-sm font-bold text-blue-100"><span>TOTAL LIABILITAS & EKUITAS</span><span className="tabular-nums">{money(pasiva)}</span></div></div>
      </div>
    </div>
  </div>;
}
...
export function LabaRugiReport(){
  ...
  if(!rows.length)return <Empty report="Laba Rugi"/>;
  ...
  return <div className="space-y-5">
    ...
    <div className="mx-auto max-w-6xl overflow-hidden rounded-xl border border-[#203047] bg-[#0a1522]">
      ...
      <IncomeSection label="PENDAPATAN" rows={revenue} subtotal="TOTAL PENDAPATAN"/>
      <IncomeSection label="HARGA POKOK PENJUALAN (HPP)" rows={hpp} subtotal="TOTAL HPP" emptyText="Tidak ada akun HPP pada periode ini."/>
      <HighlightLine label="LABA KOTOR" value={gross}/>
      <IncomeSection label="BEBAN OPERASIONAL" rows={operating} subtotal="TOTAL BEBAN OPERASIONAL" emptyText="Tidak ada beban operasional pada periode ini."/>
      <IncomeSection label="BEBAN LAIN-LAIN" rows={nonOperating} subtotal="TOTAL BEBAN LAIN-LAIN" emptyText="Tidak ada beban lain-lain pada periode ini."/>

...
export function PageHeader({title,subtitle,onUpload,slug='dashboard',extra}:{title:string;subtitle:string;onUpload:()=>void;slug?:string;extra?:React.ReactNode}){
  ...
  const filterText=[['Periode',filters.period==='all'?'Semua Periode':filters.period],['Bulan',filters.month==='all'?'Semua Bulan':monthNames[Number(filters.month)-1]],['Tahun',filters.year==='all'?'Semua Tahun':filters.year],['Company',filters.company_id==='all'?brand.name:filters.company_id],['Department',filters.department_id==='all'?'Semua Department':filters.department_id],['Cost Center',filters.cost_center_id==='all'?'Semua Cost Center':filters.cost_center_id]];

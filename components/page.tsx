...
const templateColumns:Record<string,string[]>={
  ...
  'budget-remaining':['Periode','Bulan','Tahun','Company','Department','Cost Center','Kode Akun','Nama Akun','Kategori','Budget'],
...
export function ModulePage({title,subtitle,slug}:{title:string;subtitle:string;slug:string}){
  const [open,setOpen]=useState(false),[labaRugiPeriod,setLabaRugiPeriod]=useState<LabaRugiPeriodSelection|null>(null);const [tab,setTab]=useState<'data'|'history'>('data');const financialReport=slug==='neraca'||slug==='laba-rugi'||slug==='arus-kas';const budgetType=slug==='ringkasan-budget'?'summary':slug==='budget-vs-actual'?'comparison':slug==='budget-department'?'department':slug==='budget-account'?'account':slug==='budget-remaining'?'remaining':slug==='upload-budget'?'summary':null;

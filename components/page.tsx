...
import {belongsToRestaurant,downloadFinancialReport,restaurantBrand} from '@/lib/financial-report-export';
...
export function PageHeader({title,subtitle,onUpload,slug='dashboard',extra}:{title:string;subtitle:string;onUpload:()=>void;slug?:string;extra?:React.ReactNode}){
  ...
  const runExport=async()=>{if(exporting||!can(slug,'export'))return;if(!exportRows.length){setMessage('Tidak ada data untuk diekspor.');return}setExporting(true);setMessage('');try{const statement=slug==='neraca'||slug==='laba-rugi'||slug==='arus-kas';if(statement)await downloadFinancialReport(slug,exportRows as FinancialTransaction[],filters,title,brand);else{const isBudget=slug.includes('budget');downloadExcel(isBudget?budgetColumns:transactionColumns,exportRows as Record<string,string|number|Date>[],excelFilename(title,filters.period,filters.company_id),title)}}finally{setExporting(false)}};

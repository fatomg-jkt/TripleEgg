...
import {belongsToRestaurant,downloadDashboard,downloadFinancialReport,downloadMultiPeriodLabaRugi,restaurantBrand} from '@/lib/financial-report-export';
...
export function PageHeader({title,subtitle,onUpload,slug='dashboard',extra,labaRugiPeriod}:{title:string;subtitle:string;onUpload:()=>void;slug?:string;extra?:React.ReactNode;labaRugiPeriod?:LabaRugiPeriodSelection|null}){
  ...
  const exportRows=useMemo(()=>{const exportFilters=slug==='laba-rugi'&&labaRugiPeriod?{...filters,period:'all',month:'all',year:'all'}:filters,allowed=inScope(applyFilters(transactions,exportFilters),user),scopedTransactions=allowed.filter(row=>belongsToRestaurant(row.company_id,brand));if(slug.includes('budget')){const scopedBudgets=inScope(applyFilters(budgets,filters),user);return scopedBudgets.map(row=>{const actual=scopedTransactions.filter(tx=>tx.account_code===row.account_code&&(row.department_id==='Semua Department'||tx.department_id===row.department_id)&&(row.cost_center_id==='Semua Cost Center'||tx.cost_center_id===row.cost_center_id)).reduce((sum,tx)=>sum+signed(tx),0);return {...row,actual,variance:row.budget-actual}})}const statement=slug==='neraca'?'balance-sheet':slug==='laba-rugi'?'income-statement':slug==='arus-kas'?'cash-flow':null;if(statement)return scopedTransactions.filter(row=>row.statement_type===statement);return scopedTransactions.map(row=>({...row,transaction_date:row.transaction_date?new Date(`${row.transaction_date}T00:00:00`):'',report_category:row.report_category||row.description,balance:signed(row)}))},[transactions,budgets,filters,user,slug,brand,labaRugiPeriod]);
  const runExport=async()=>{if(exporting||!can(slug,'export'))return;if(slug!=='dashboard'&&!exportRows.length){setMessage('Tidak ada data untuk diekspor.');return}setExporting(true);setMessage('');try{const statement=slug==='neraca'||slug==='laba-rugi'||slug==='arus-kas';if(slug==='dashboard')await downloadDashboard(exportRows as FinancialTransaction[],filters,brand);else if(slug==='laba-rugi'&&labaRugiPeriod)await downloadMultiPeriodLabaRugi(exportRows as FinancialTransaction[],filters,brand,labaRugiPeriod);else if(statement)await downloadFinancialReport(slug,exportRows as FinancialTransaction[],filters,title,brand);else{const isBudget=slug.includes('budget');downloadExcel(isBudget?budgetColumns:transactionColumns,exportRows as Record<string,string|number|Date>[],excelFilename(title,filters.period,filters.company_id),title)}}finally{setExporting(false)}};
  const filterText=[['Periode',labaRugiPeriod?.label||(filters.period==='all'?'Semua Periode':filters.period)],['Bulan',labaRugiPeriod?'Dikontrol mode multi-periode':(filters.month==='all'?'Semua Bulan':monthNames[Number(filters.month)-1])],['Tahun',filters.year==='all'?'Semua Tahun':filters.year],['Company',filters.company_id==='all'?brand.name:filters.company_id],['Department',filters.department_id==='all'?'Semua Department':filters.department_id],['Cost Center',filters.cost_center_id==='all'?'Semua Cost Center':filters.cost_center_id]];
  return <><section className="print-report-header"><img src={brand.logoPath} alt={brand.name}/><div><b>FINANCIAL &amp; ACCOUNTING</b><span>{brand.name}</span><h1>{title.toUpperCase()}</h1><div className="print-filter-summary">{filterText.map(([label,value])=><span key={label}><b>{label}:</b> {value}</span>)}</div></div></section><div className="no-print flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><div className="label mb-2 text-blue-500">Finance / Overview</div><h1 className="text-2xl font-bold tracking-tight">{title}</h1><p className="mt-1 text-xs text-slate-500">{subtitle}</p></div><div className="flex flex-wrap gap-2">{extra}<PermissionGate resource={slug} action="upload"><button onClick={onUpload} className="btn btn-primary"><Upload size={14}/> Upload File</button></PermissionGate><PermissionGate resource={slug} action="export"><button onClick={()=>downloadTemplate(title,slug)} className="btn" type="button"><FileDown size={14}/> Download Template</button><button onClick={runExport} disabled={exporting} className="btn" type="button"><Download size={14}/> {exporting?'Exporting...':'Export Excel'}</button></PermissionGate><button onClick={()=>window.print()} className="btn desktop-only" type="button"><Printer size={14}/> Print</button></div></div>{message&&<p role="status" className="no-print mt-3 rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2 text-xs text-amber-300">{message}</p>}</>;
}

export function ModulePage({title,subtitle,slug}:{title:string;subtitle:string;slug:string}){
  const [open,setOpen]=useState(false),[labaRugiPeriod,setLabaRugiPeriod]=useState<LabaRugiPeriodSelection|null>(null);const [tab,setTab]=useState<'data'|'history'>('data');const financialReport=slug==='neraca'||slug==='laba-rugi'||slug==='arus-kas';const budgetType=slug==='ringkasan-budget'?'summary':slug==='budget-vs-actual'?'comparison':slug==='budget-department'?'department':slug==='budget-account'?'account':slug==='upload-budget'?'summary':null;

...
function detectStatement(report:string):StatementType|null{const value=normalized(report);if(!/\bmulti(?:ple)?\s+period\b/.test(value))return null;if(value.includes('balance sheet'))return'balance-sheet';if(value.includes('profit/loss')||value.includes('profit or loss'))return'income-statement';if(value.includes('cash flow'))return'cash-flow';return null}

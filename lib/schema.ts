...
export interface FinancialTransaction{id:string;transaction_date:string;account_code:string;account_name:string;account_type:AccountType;debit:number;credit:number;company_id:string;department_id:string;cost_center_id:string;period:string;month:number;year:number;description:string;source_file_id:string;created_at:string}
export interface ImportRecord extends UploadedFile{rows_imported:number;rows_failed:number;raw_data?:string}

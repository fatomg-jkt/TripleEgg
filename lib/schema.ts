...
export type Role='Owner'|'Super Admin'|'Finance Manager'|'Accounting'|'Staff'|'Viewer';
export type PermissionAction='view'|'upload'|'edit'|'delete'|'export'|'approve';
export type Permission=PermissionAction;
export type RestaurantSlug='triple-egg'|'wok-this-way';
export interface Restaurant{id:string;slug:RestaurantSlug;name:'Triple Egg'|'Wok This Way'}
...
export interface BudgetRecord{id:string;restaurant_id?:string;period:string;month:number;year:number;company_id:string;department_id:string;cost_center_id:string;account_code:string;account_name:string;category:string;budget:number;source_file_id:string;created_at:string}
